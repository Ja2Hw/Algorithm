...
def solution(k, tangerine):
    ...
    tmp_list.sort(reverse=True, key = lambda x:x[1])    
    #print(tmp_list)
...
'''
    tan = sorted(tangerine)
    print(tan)
    
#    while (len(tan) > k):
    for i in range(len(tan)-k):
        print("for", tan)
        if i == 0:
            tmp = len(set(tan[i: i+k]))
            print("start", tan[i: i+k])
        elif tmp > len(set(tan[i: i+k])):
            tmp = len(set(tan[i: i+k]))
            print(tan[i: i+k])
